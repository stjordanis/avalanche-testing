...
func CreateIndependentBurnTxs(utxos []*avax.UTXO, utxoAmount, txFee uint64, privateKey *crypto.PrivateKeySECP256K1R) ([][]byte, []ids.ID, error) {
	txBytes := make([][]byte, 0, len(utxos))
	txIDs := make([]ids.ID, 0, len(utxos))
